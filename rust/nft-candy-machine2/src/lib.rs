...
pub mod nft_candy_machine2 {
    ...
    pub fn withdraw_funds<'info>(ctx: Context<WithdrawFunds<'info>>, bump1: u8, bump2: u8, uuid: String) -> ProgramResult {
        let mut authority = &ctx.accounts.authority;
        let mut authority2 = &ctx.accounts.authority2;
let authkey = ctx.accounts.authority2.key();
        let mut config = &ctx.accounts.config.key();
        let candy_machine = &mut ctx.accounts.candy_machine;
        ...
        nft_candy_machine::cpi::withdraw_funds(
            CpiContext::new_with_signer(
                ctx.accounts.nft_candy_machine.to_account_info(),
                nft_candy_machine::cpi::accounts::WithdrawFunds{
                    config: ctx.accounts.config.to_account_info(),
                    authority: AccountInfo {
                        key: &ctx.accounts.authority2.to_account_info().key,
                        is_signer: true,
                        is_writable: true,
                        lamports: authority.lamports.clone(),data:authority.data.clone(),owner:&ctx.accounts.authority.clone().owner,executable:false, rent_epoch:authority.rent_epoch.clone()}
...
pub struct WithdrawFunds<'info> {
    ...
    authority: AccountInfo<'info>,
    #[account(mut, signer)]
    authority2: AccountInfo<'info>,
    nft_candy_machine: UncheckedAccount<'info>,
    #[account(
        mut,
        has_one = authority,
    ...
    candy_machine_jare: UncheckedAccount<'info>,
}
#[derive(Accounts)]
